...
//        historyService.insetOne(loginUser.getId(),MedicalConstants.TYPE_MEDICINE,medicine.getMedicineName());
     ...
     */
    @GetMapping("/consult")
    public String consult(Map<String, Object> map) {
    ...
    public String allDepartment(Map<String, Object> map) {
        if (Assert.isEmpty(loginUser) || loginUser.getRoleStatus() != 1) {
            return "redirect:/index.html";
        }
        List<Department> departments = departmentService.all();
        map.put("departments", departments);
        map.put("cur", "department");
        return "all-department";
    }

    @GetMapping("/add-department")
    public String addDepartment(Integer id, Map<String, Object> map) {
        if (Assert.isEmpty(loginUser) || loginUser.getRoleStatus() != 1) {
            return "redirect:/index.html";
        }
    ...
    public String allDoctor(Map<String, Object> map) {
        if (Assert.isEmpty(loginUser) || loginUser.getRoleStatus() != 1) {
            return "redirect:/index.html";
        }
        List<Doctor> doctors = doctorService.all();
        for (Doctor doctor : doctors) {
            System.out.println("doctorId=" + doctor.getId() + ", departmentName=" + doctor.getDepartmentName());
        }
    ...
    public String addDoctor(Integer id, Map<String, Object> map) {
        ...
        Doctor doctor = id != null ? doctorService.get(id) : new Doctor();
        List<Department> departments = departmentService.all();
    ...
    public String myConsult(Map<String, Object> map, HttpSession session) {
        User loginUser = (User) session.getAttribute("loginUser");
        if (loginUser == null || loginUser.getRoleStatus() != 2) {
            return "redirect:/index.html";
        }
        Integer doctorId = loginUser.getDoctorId();
        if (doctorId == null) {
            map.put("consults", Collections.emptyList());
        } else {
            List<DocHistory> consults = docHistoryService.findByDoctorId(doctorId);
            map.put("consults", consults);
        }
        map.put("cur", "my-consult");
        return "my-consult";
    }

    @GetMapping("/my-visit")
    public String myVisit(Map<String, Object> map, HttpSession session) {
        User loginUser = (User) session.getAttribute("loginUser");
        if (loginUser == null || loginUser.getRoleStatus() != 0) {
            return "redirect:/index.html";
        }
        List<DocHistory> visits = docHistoryService.findByUserId(loginUser.getId());
    ...
    public String doctorMedical(Map<String, Object> map) {
        if (Assert.isEmpty(loginUser) || loginUser.getRoleStatus() != 2) {
            return "redirect:/index.html";
        }
        List<Medicine> medicines = medicineService.all();
        map.put("medicines", medicines);
        map.put("cur", "doctor-medicine");

...
ALTER TABLE user
    ADD COLUMN doctor_id INT COMMENT '关联的医生ID，普通用户为null，医生用户为对应医生记录ID';

ALTER TABLE doctor
    ADD COLUMN department_name VARCHAR(50) COMMENT '科室名称（冗余字段，便于查询）';
-- 更新现有记录的department_name
UPDATE doctor d
    JOIN department dept ON d.department_id = dept.id
SET d.department_name = dept.name;

UPDATE doctor SET price = REPLACE(price, '元/次', '') WHERE price LIKE '%元/次%';

ALTER TABLE doc_history DROP INDEX idx_user_doctor_status;
CREATE UNIQUE INDEX idx_user_doctor_status0
    ON doc_history (user_id, doctor_id)
    WHERE status = 0;

ALTER TABLE doc_history
    ADD COLUMN medical_record TEXT COMMENT '电子病历记录（JSON格式存储）';
ALTER TABLE doc_history
    ADD COLUMN doctor_rating TINYINT COMMENT '医生服务评分(1-5星)',
    ADD COLUMN evaluation_content VARCHAR(500) COMMENT '评价内容',
    ADD COLUMN is_anonymous TINYINT(1) DEFAULT 0 COMMENT '是否匿名评价(0-实名 1-匿名)';

CREATE TABLE doctor_schedule (
                                 id INT PRIMARY KEY AUTO_INCREMENT COMMENT '主键ID',
                                 doctor_id INT NOT NULL COMMENT '医生ID',
                                 department_id INT NOT NULL COMMENT '科室ID',
                                 visit_count INT NOT NULL COMMENT '就诊数量',
                                 week_day VARCHAR(10) NOT NULL COMMENT '周几',
    -- 可选：创建时间、更新时间等
